...
namespace PricingEngine.Tests
{
    ...
    /// </summary>
    [TestClass]
    public class PromotionDecorator_DoPricing

    {
        ...
        [TestMethod]
        public void WhenInputContainsPriceWhichIsLessThan50PercentageOfAveragePrice_DoNotConsider()
        {
            ...
            IPricer pricer = new PromotionDecorator(mockPricer.Object);
            Product product = pricer.DoPrice(specs, compList);

            mockPricer.Verify(m => m.DoPrice(specs,
                                             It.Is<IEnumerable<CompetitorProduct>>(s => s.Count() == 2))
                            );
        }
    }
}
